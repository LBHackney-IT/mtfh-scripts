...
from aws.database.rds.housing_finance.EE.entities.GoogleFileSetting import (
...
def session_for_hfs(
    ...
    try:
        ...
        raise IndexError(
            "No ODBC Driver 17 for SQL Server found. Please install the driver and try again."
        )

    connection_string = f"DRIVER={driver};SERVER=127.0.0.1,{local_port};DATABASE=sow2b;UID={username};PWD={password}"
    engine = create_engine(
        "mssql+pyodbc://", creator=lambda: pyodbc.connect(connection_string), echo=True
    )
...
if __name__ == "__main__":
    ...
    with HfsSession.begin() as session:
        res = (
            ...
            .where(GoogleFileSetting.Label == "ChargesOLD")
            .all()
        )
